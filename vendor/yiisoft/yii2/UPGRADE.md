...
* The `yii\console\rest\MessageController` class has been refactored to be better configurable and now also allows
...
* The signature of the following methods in `yii\console\rest\MessageController` has changed. They have an extra parameter `$markUnused`.
...
* `yii\console\rest\AssetController` is now using hashes instead of timestamps. Replace all `{ts}` with `{hash}`.
