...
- Bug #11280: Descendants of `yii\console\rest\BaseMigrateController`, like the one for MongoDB, unable to create new migration (klimov-paul)
...
- Bug #9323: Fixed `yii\console\rest\MessageController` not using database connection specified in config (raccoon69, samdark)
...
- Enh #10359: Support wildcard category name in `yii/console/rest/MessageController` (rmrevin)
...
- Enh #8070: `yii\console\rest\MessageController` now sorts created messages, even if there is no new one, while saving to PHP file (klimov-paul)
- Enh #8286: `yii\console\rest\MessageController` improved allowing extraction of nested translator calls (klimov-paul)
...
- Enh #8574: Added `yii\console\rest\MessageController` support .pot file creation (pgaultier)
- Enh #8625: Added `markUnused` option to `yii\console\rest\MessageController` (marius7383)
...
- Bug #7473: Fixed `yii\console\rest\AssetController` does not create missing folders for the target bundles (schmunk42, klimov-paul)
...
- Enh #7539: `yii\console\rest\AssetController` provides dependency trace in case bundle circular dependency detected (klimov-paul)
...
- Bug #7037: `yii\console\rest\AssetController` now correctly handles relative image URLs if source and target CSS are under same directory (klimov-paul)
...
- Enh #7269: `yii\console\rest\BaseMigrateController` now throws exception if directory specified doesn't exist and action isn't `create` (lynicidn, samdark)
...
- Enh #5194: `yii\console\rest\AssetController` now handles bundle files from external resources properly (klimov-paul)
...
- Bug #5521: Fixed `yii\console\rest\AssetController` breaks CSS URLs, which start from '/' (klimov-paul)
...
- Bug #4881: Fixed `yii\console\rest\AssetController` breaks CSS URLs on Windows (klimov-paul)
- Bug #5171: Fixed the bug that ActiveForm + Pjax submit event is only triggered once (qiangxue)
- Bug #5242: Fixed `yii\console\rest\AssetController` breaks CSS URLs in case target file localed at `yii\web\AssetManager::basePath` root (klimov-paul)
...
- Bug #2821: Console help command incorrectly lists non-console rest as available commands (qiangxue)
...
- Bug #3125: `yii\console\rest\AssetController` now respects data URL resources (klimov-paul)
...
- Bug #4926: Fixed `yii\console\rest\MessageController` handles category name containing dot incorrectly (klimov-paul)
...
- Bug #5055: Fixed `yii\console\rest\CacheController` does not check if cache component instance of 'yii\caching\Cache' (klimov-paul)
...
- Enh #3801: Base migration controller `yii\console\rest\BaseMigrateController` extracted (klimov-paul)
...
- Chg #2898: `yii\console\rest\AssetController` is now using hashes instead of timestamps (samdark)
...
- Enh #1467: Added support for organizing rest in subdirectories (qiangxue)
